...
EVAL_POLS = ["payday_wait", "solo_shared_pd", "ml_index", "oracle"]
...
def stage_eval():
    ...
    for wname, _ in WORLDS:
        row = {}
        for pol in EVAL_POLS:
            row[pol] = [res[f"{wname}|{ps}|{pol}"]["cycle_rec"]
                        for ps in EVAL_POPS]
        table.append((wname, row))
        out[wname] = {p: float(np.mean(v) * 100) for p, v in row.items()}

    w0 = max(len(w) for w, _ in WORLDS) + 2
    print(f"{'world':<{w0}}" + "".join(f"{p:>17}" for p in EVAL_POLS)
          + f"{'ml - bayes':>14}")
    for wname, row in table:
        line = f"{wname:<{w0}}"
        for p in EVAL_POLS:
            line += f"{np.mean(row[p])*100:>16.2f}%"
        m, e = paired(row["solo_shared_pd"], row["ml_index"])
        sig = "SIG" if abs(m) > e else "n.s."
        line += f"  {m:+6.2f}+/-{e:.2f} {sig}"
        print(line)
        out[wname]["ml_minus_bayes"] = [m, e, sig]
...
if __name__ == "__main__":
    ...
    {"data": stage_data, "train": stage_train, "eval": stage_eval}[cmd]()
